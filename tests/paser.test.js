...
describe('parseLine', () => {
    ...
    test('should return null for empty line', () => {
        const result = (0, parser_1.parseLine)('');
        expect(result).toBeNull();
    });
    test('should return null for invalid numeric fields', () => {
        const invalidLine = '9781234567890' +
            'Invalid Book'.padEnd(100, ' ') +
            'Unknown'.padEnd(50, ' ') +
            'abcd' + // Invalid PublishedYear
            'xyz'; // Invalid Copies
        const result = (0, parser_1.parseLine)(invalidLine);
        expect(result).toBeNull();
    });
    test('should return null when ISBN is missing', () => {
        const invalidLine = ''.padEnd(13, ' ') + // Empty ISBN
    ...
    test('should trim spaces correctly from fields', () => {
        const spacedLine = '9780000000000' +
            '   A Spaced Title   '.padEnd(100, ' ') +
            '   An Author   '.padEnd(50, ' ') +
            '1999' +
            '005';
        const result = (0, parser_1.parseLine)(spacedLine);
